...
def _extract_vector_image(picture: dict[str, Any] | None) -> dict[str, Any] | None:
    """Extract vectorImage from nested PhotoFilterPicture or picture dictionary."""
    if not picture or not isinstance(picture, dict):
        return None

    # Standalone PhotoFilterPicture entity
...
def _split_geo_display(display: str | None) -> tuple[str | None, str | None, str | None]:
    """Extract city, state, country from formatted 'City, State, Country' strings."""
    if not display:
...
def skills_paging_info(raw: dict[str, Any]) -> tuple[int, int, str | None]:
    """Extract (total, existing_skill_count, profile_urn) from raw Voyager response."""
...
def merge_skill_entities(raw: dict[str, Any], extra_skills: list[dict[str, Any]]) -> dict[str, Any]:
    """Append newly paginated Skill entities into raw.included without duplicates."""
    if not extra_skills:
        return raw
...
def parse_profile_response(raw: dict[str, Any]) -> dict[str, Any]:
    """Transform raw Voyager graph entities into normalized ProfileResponse dictionary."""
    included = raw.get("included") or []
    profile = _find_profile(included, raw)
